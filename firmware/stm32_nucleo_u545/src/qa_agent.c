 ...
 * Direct register access for reliable byte-by-byte reading
 */
void qa_agent_poll(void)
{
    ...
    // Read bytes while RXNE flag is set (data available)
    // Use direct register access for LPUART
    while (__HAL_UART_GET_FLAG(s_huart, UART_FLAG_RXNE) != RESET) {
        uint8_t byte;
        
        // Read directly from RDR register (clears RXNE flag automatically)
        if (s_huart->Instance == LPUART1) {
            byte = (uint8_t)(READ_REG(s_huart->Instance->RDR) & 0xFF);
        } else {
            // For regular UART
            byte = (uint8_t)(READ_REG(s_huart->Instance->RDR) & 0xFF);
        }
